    ...
    <section className="bg-[#f7f7f7] py-8 md:py-16">
      <div className="max-w-[1280px] mx-auto px-4 md:px-8">
        <div className="flex flex-col md:relative min-h-[300px] md:min-h-[483px] md:flex-row md:items-center gap-6 md:gap-0">
          {/* Mobile: Top Image */}
          <div className="md:hidden rounded-[12px] overflow-hidden border border-[#f3f3f3] shadow-md">
            <img
              src={image}
              alt={name + " - " + role}
          ...
          <div
            className={`bg-white border-8 md:border-10 border-white rounded-[12px] md:rounded-[16px] shadow-lg flex flex-col gap-3 md:gap-4 px-4 md:px-[20px] py-5 md:py-[20px] w-full md:max-w-[560px] z-10 relative ${
              isCardLeft ? "md:order-1" : "md:order-1 md:ml-auto"
            }`}
          >
          ...
          {/* Athlete Image - Desktop */}
          <div
            className={`hidden md:block md:absolute md:top-0 md:w-[720px] md:h-[493px] rounded-[20px] border border-[#f3f3f3] overflow-hidden bg-white shadow-md ${
              isCardLeft ? "md:right-0" : "md:left-0"
            }`}
          >
            <img
              src={image}
